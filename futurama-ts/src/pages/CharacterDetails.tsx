...
const CharacterDetails = () => {
  ...
  const handleComment = (userInput: string, setUserInput: React.Dispatch<React.SetStateAction<string>>) => {
  ...
  return (
    <div style={{display: "flex", flexFlow: "column wrap", alignItems: "center", gap: "1rem"}}>
      ...
      <div style={{display: "flex", flexFlow:"column wrap", border: "1px black solid", width:"90%", }}>
        { comments.map((c) => {
          return (
            <div style={{display: "grid", placeItems:"center", border:"1px solid black", gap:"0", margin:"10px", backgroundColor:"#4AE0FF", padding:"10px"}}>
              <p style={{fontWeight: "bold"}}>Username: {c.user}</p>
              <pre>{c.comment}</pre>
              <p>{c.date}</p>
            </div>
          )
